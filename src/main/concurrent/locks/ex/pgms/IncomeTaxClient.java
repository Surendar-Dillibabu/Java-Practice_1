...
public class IncomeTaxClient {
  ...
  private static IncomeTaxDept incomeTaxDept = new IncomeTaxDept(100, NUMBER_OF_TAX_PAYER);
    ...
    for (TaxPayer taxPayer : incomeTaxDept.getTaxPayersList())
      executor.submit(() -> {
        try {
          ...
          revenue = incomeTaxDept.getTotalRevenueOptimisticRead();
          LOG.info(
              "IncomeTax Department's total revenue after getting State tax return at client code is --->> " + revenue);
          Thread.sleep(100);
        } catch (Exception e) {
        }
      });
